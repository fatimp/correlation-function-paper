...
preferred than $O(MC \log M)$ algorithm. This algorithm is present below:
\begin{algorithmic}[1]
...
many of them can be parralelized on GPU. In \code{chord\_length} function we
firstly apply distance transform to an input to find the interface and then
accumulate chord length in per-slice manner. Finally, cord lengths can be binned
in a histogram.
\begin{algorithmic}[1]
